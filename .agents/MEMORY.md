...
- **[2026-09-05]** — **GitHub Actions: Purge automático restaurado** (`5839051`, `35b877d`): workflow `purge-cache.yml` creado (se había perdido). Se dispara tras cada deploy exitoso de GitHub Pages. Requiere secrets `CLOUDFLARE_ZONE_ID`, `CLOUDFLARE_API_TOKEN` y `CLOUDFLARE_EMAIL` configurados. **Estado: secrets configurados por el usuario.**
- **[2026-09-05]** — **Secrets de Cloudflare configurados** — el usuario creó los 3 secrets en GitHub (CLOUDFLARE_ZONE_ID, CLOUDFLARE_API_TOKEN, CLOUDFLARE_EMAIL). El workflow purge-cache.yml funciona con API Token (formato Bearer).
...
- **Branch:** main
- **Último commit desplegado:** `35b877d` (docs update) — deploy exitoso.
- **Redes sociales:** Instagram `@yo_soy222`, TikTok `@yo_soy222`, Facebook `share/1C5X2yKscG/`.
- **GitHub Actions:** Workflow `purge-cache.yml` configurado y funcionando. Secrets: `CLOUDFLARE_ZONE_ID`, `CLOUDFLARE_API_TOKEN` (API Token con Bearer), `CLOUDFLARE_EMAIL` — todos configurados.
...
- **[2026-09-05]** — **Documentación completa actualizada** (`AGENTS.md`, `MEMORY.md`, `README.md`, `PLAN_IMPLEMENTACION.md`): nuevo archivo AGENTS.md con instrucciones para agentes AI; MEMORY.md v6 con estado actual; README.md con datos correctos (imágenes hero, cache v3, línea counts, 60 imágenes por carpeta).
...
- [x] **Configurar secrets de Cloudflare en GitHub** — COMPLETADO (usuario configuró ZONE_ID, API_TOKEN, EMAIL).
