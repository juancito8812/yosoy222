...
- **[2026-09-05]** — **PWA offline total + Cloudflare Polish**: `sw.js` bump v3→v4 con mensaje `PRECACHE_IMAGES` (la página envía thumbs+catalog de los 44 productos y el SW los cachea en segundo plano, idempotente con marcador `CATALOG_MARKER`); `app.js` lo dispara en `updatefound`/`controllerchange`. Cloudflare Polish habilitado (WebP automático en el edge).
