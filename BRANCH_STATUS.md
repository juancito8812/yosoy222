...
- **Preview permanente `preview.yosoy222.com` (debianm700) — DOS LINKS FIJOS (25-sep):**
  1. **`https://preview.yosoy222.com/`** → rediseño v42 con las portadas corregidas del dueño (espejo re-sincronizado `imgVer` 12; backups en `site-backup-v40` y `site-backup-v42-previas`)
  2. **`https://preview.yosoy222.com/legal-portal/legal/terminos.html`** → bloque legal de main (4 páginas, marcadores `[COMPLETAR]` visibles), montado como **volumen persistente aparte** `~/yosoy222-preview/legal-portal` (sobrevive a las re-sincronizaciones del espejo); nginx con `root /srv/site` + `alias /srv/legal-portal/` (los mounts van hermanos bajo `/srv` — anidar un mount dentro de otro mount falla con read-only FS). El "Volver a la tienda" del portal apunta a `../../` (producción)
- Ambos links viven mientras debianm700 esté encendido — independientes de la PC del agente. **PENDIENTE que los bloquea: caída de internet en debianm700 desde la mañana del 25-sep** (WAN 000, dominio 530); al volver, el túnel se registra solo (`restart: always`) y ambos links funcionan sin más pasos; si el 530 persistiera unos minutos, purgar en Cloudflare
- Reparado también el crash-loop del conector: la imagen cloudflared corre como uid 65532 y el token era 600 del uid 1000 → `chgrp 65532 + chmod 640` (vía contenedor alpine, sin sudo). Actualizar el espejo tras cambios futuros: tar (excluir `.git`, `.agents`, `BRANCH_STATUS.md`, `PLAN_IMPLEMENTACION.md`, `tests`, `scripts`, `supabase`, `package*.json`) hacia `~/yosoy222-preview/site/` + `docker compose up -d --force-recreate previewer` (el bind mount queda anclado al inodo viejo si solo se renombra la carpeta)
